      ...
      <SafeAreaView style={styles.modalSafe}>
        ...
        <View style={styles.dateGrid}>
          {DATES.map(d => (
            <TouchableOpacity
              key={d}
              style={[styles.dateCell, selected === d && styles.dateCellActive]}
...
function HoraModal({ visible, onClose, onAdd }) {
  ...
  return (
    <Modal visible={visible} animationType="slide">
      <SafeAreaView style={styles.modalSafe}>
        <View style={styles.modalHeader}>
          <Text style={styles.modalTitle}>Selecciona horas disponibles</Text>
          <TouchableOpacity onPress={onClose} hitSlop={{ top: 10, bottom: 10, left: 10, right: 10 }}>
            <Ionicons name="close" size={24} color={colors.textPrimary} />
          </TouchableOpacity>
        </View>
        <View style={styles.hourGrid}>
          {HOURS.map(h => (
            <TouchableOpacity
              key={h}
              style={[styles.hourCell, selected === h && styles.hourCellActive]}
...
function PartidoModal({ visible, onClose, onAdd }) {
  ...
  return (
    <Modal visible={visible} animationType="slide">
      <SafeAreaView style={styles.modalSafe}>
        <View style={styles.modalHeader}>
          <Text style={styles.modalTitle}>Selecciona tipo de partido</Text>
          <TouchableOpacity onPress={onClose} hitSlop={{ top: 10, bottom: 10, left: 10, right: 10 }}>
            <Ionicons name="close" size={24} color={colors.textPrimary} />
          </TouchableOpacity>
        </View>
    ...
    <SafeAreaView style={styles.safe}>
      ...
      <View style={styles.bottomBar}>
        <TouchableOpacity
          ...
          onPress={() => navigation.navigate('CrearPartido', { tipo: activeTab })}
        >
      ...
      <CanchaModal
        visible={openModal === 'cancha'}
        onClose={() => setOpenModal(null)}
        onAdd={v => applyFilter('cancha', v)}
      />
      <FechaModal
        visible={openModal === 'fecha'}
        onClose={() => setOpenModal(null)}
        onAdd={v => applyFilter('fecha', v)}
      />
      <HoraModal
        visible={openModal === 'hora'}
        onClose={() => setOpenModal(null)}
        onAdd={v => applyFilter('hora', v)}
      />
      <PartidoModal
        visible={openModal === 'partido'}
        onClose={() => setOpenModal(null)}
        onAdd={v => applyFilter('partido', v)}
      />
    </SafeAreaView>
  );
}
